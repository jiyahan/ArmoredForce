...
// On Windows, stub out some Unix-only functionality.
namespace RCF {

    class RCF_EXPORT UnixLocalEndpoint : public Endpoint
    {
    public:
        UnixLocalEndpoint(const std::string & socketName)                       { RCF_THROW(Exception(_RcfError_NotSupportedOnWindows("UnixLocalEndpoint"))); }
...
// On Unix, stub out some Windows-only functionality.
namespace RCF {
...
public:
    ...
    RcfProtoController();
    RcfProtoController(RcfProtoSession * context);
...
private:
    ...
    void ProtoRpcEndCpp(
        std::pair<
            ...
            google::protobuf::Message *>    requestResponsePair, 
